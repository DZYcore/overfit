...
def parse_args():
    ...
    parser.add_argument(
        "--npz_path",
        type=str,
        default="./results_fl_cifar10/classwise_metrics.npz",
        help="Path to the saved classwise_metrics.npz file",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./results_fl_cifar10/per_class_plots",
...
# 新增: 兼容性检查——判断是否存在 test_class_accuracy
...
for c in range(num_classes):
    ...
    # 新增: Test Accuracy 曲线,同样画在左轴(和Train/Calib Acc同一个量纲)
    if has_test_acc:
        l4, = ax_acc.plot(
            rounds, test_acc[:, c],
    ...
    # --- 坐标轴与细节设置 ---
    ax_acc.set_xlabel("Federated Round", fontsize=12)
    ...
    # 合并图例到右上角
    # lines = [l1, l2, l3]   # 原代码保留(注释掉): 不包含 Test Acc
    lines = [l1, l2, l4, l3] if has_test_acc else [l1, l2, l3]  # 新增: 有test数据时加入l4
